-- Baseline schema for migration 0000_baseline_schema.
-- Derived from database/magellon01db.sql: DDL only. Stripped:
-- INSERT/LOCK/DROP statements, CREATE DATABASE and USE (tables must
-- land in the schema the alembic connection points at).
-- Regenerate with the same filter if the dump is ever refreshed;
-- the dump must stay a pre-0001 snapshot.
...
- Default admin user (super/the dev-stack default)
...
-- Default admin user: super / the dev-stack default (bcrypt hashed)
...
Default login credentials:
  Username: super
  Password: the dev-stack default
