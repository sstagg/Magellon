...
# ============================================
# boxnet_pt.py (modified)
# ============================================
def pad_to_unit_3d(img_3d_np, pad_unit = 256):
    """
    Pads a 3D NumPy array (B, H, W) to a multiple of 32 size
    """
    # 1. Get original dimensions and compute target size
    ...
    # 2. Calculate padding amounts
    H_pad = H_target - H_orig
    W_pad = W_target - W_orig

    pad_top = H_pad // 2
    pad_left = W_pad // 2

    padded_imgs = np.zeros((len(img_3d_np), H_target, W_target))
    padded_imgs[:, pad_top:pad_top + H_orig, pad_left:pad_left + W_orig] = img_3d_np

    return padded_imgs


def unpad_batch(nparray_4d, H: int, W: int):
    """
    Unpads a 4D NumPy array (B, H, W, C) 
    """
    ...
    return nparray_4d[:, top:bottom, left:right, :]


def unpad_img(nparray_3d, H: int, W: int) -> NDArray:
    """
    Center-crop (remove padding) from a large image to target size (H, W).

    Parameters
    ----------
    img : np.ndarray
        Input 2D (grayscale) or 3D (multi-channel) image.
    H : int
        Target crop height.
    W : int
        Target crop width.
    ...
    cropped_img : np.ndarray
        Center-cropped image of shape (H, W) or (H, W, C).
    """
    h, w = img.shape[:2]
...
class BoxnetPT():
    ...
    def __init__(self, model_path: str):
        ...
        # self.model.print_readable()
        setattr(self.model, 'forward', flexible_forward.__get__(self.model, type(self.model)))
        self.model.eval()

        self.device = "cpu"

        print(f"Loaded model from {model_path} on device {self.device}")
        print(f"Use .to(device) to load on cuda")
    ...
    def compute_batch(self, imgs: NDArray) -> NDArray:
        ...
        imgs = (imgs - imgs.mean(axis=1).reshape(-1, 1)) / imgs.std(axis=1).reshape(-1, 1)
        imgs = imgs.reshape((-1, H, W))

        imgs = pad_to_unit_3d(imgs)
        
        inputs = torch.tensor(imgs, dtype=torch.float32)
        inputs = inputs.unsqueeze(1)
        inputs = inputs.to(self.device)

        # Compute masks with Boxnet v2
...
if __name__ == "__main__":
    ...
    img = np.array(raw_img)[:490, :490]

    # model = BoxnetPT("boxnet.pt").to("mps")
    model = BoxnetPT("boxnet.pt")

    # Boxnet
    inputs = np.array([img, np.rot90(img), np.rot90(np.rot90(img)), np.rot90(np.rot90(np.rot90(img)))])
    ...
    for i, img in enumerate(inputs):
        ...
        plt.imshow(img, cmap="gray")
        plt.imshow(result[:, :, 1], cmap="jet", alpha=0.3)
        plt.title("Particle mask")
        plt.show()

        plt.imshow(img, cmap="gray")
        plt.imshow(result[:, :, 2], cmap="jet", alpha=0.3)
        plt.title("Dirt mask")
        plt.show()
