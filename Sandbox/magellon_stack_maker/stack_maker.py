"""
Atomic particle stack maker.

Given a micrograph and particle coordinates, this module extracts boxed particles,
computes edge-based per-particle normalization, pads edge boxes, and emits RELION-ready outputs.
"""
...
class ParticleStackConfig:
    """Control options for stack generation."""
...
class ParticleStackRow:
    """Output row for STAR/JSON export."""
    ...
    # Optional fields used when writing mrcs stack
...
def _half_box(box_size: int) -> int:
    ...
    if box_size % 2 != 0:
        raise ValueError("box_size must be even to use current centered extraction convention")
    return int(math.floor((box_size - 1) / 2))


def _normalize_edge_stats(box: np.ndarray, edge_width: int) -> Tuple[float, float]:
    # Use border pixels as a noise floor estimate. If the box is too narrow for
    # a conventional border stripe, fall back to all pixels.
    if edge_width <= 0:
...
def _extract_box(image: np.ndarray, x: float, y: float, box_size: int, allow_partial: bool) -> np.ndarray:
    # Picker coordinates are in full-resolution pixel units. Here we center a
    # square box on (rounded) (x, y) and optionally clip at image edges.
    if box_size <= 0:
...
def _pad_to_square(box: np.ndarray, target_shape: int) -> np.ndarray:
    # Keep full data as-is when already the requested size.
    if box.ndim != 2:
...
def _pad_to_square_with_falloff(
    box: np.ndarray,
    target_shape: int,
) -> np.ndarray:
    """
    Soft-pad partial boxes by blending toward a mean background near the padded
    border without stretching edge pixels.
    """
    if target_shape < 1:
    ...
    # Compute background level from valid pixels so NaN/Inf in padded regions do not
    # contaminate the fill value.
    finite = np.isfinite(box)
    if finite.any():
        fill = float(np.mean(box[finite]))
    else:
        fill = 0.0

    # Symmetric-ish placement with integer rounding:
    # extra row/col is pushed to the bottom/right if odd difference.
    ...
    # Initialize full target with mean fill and place observed pixels in the center.
    out = np.full((target_shape, target_shape), fill, dtype=np.float32)
    out[y0:y1, x0:x1] = box.astype(np.float32)

    # Distance from each pixel inside the extracted box to its nearest box edge.
    ...
    # Keep padded pixels fixed at mean. Taper only the last few rows/cols of the
    # real extracted region into mean to avoid a sharp boundary edge.
...
def _normalize_box(box: np.ndarray, edge_width: int) -> Tuple[np.ndarray, float, float]:
    # Normalize in-place with edge statistics and return both transformed image and
    # the background model used for that particle.
...
def _as_particle_coords(source: Sequence[Dict[str, Any]]) -> List[ParticleCoordinate]:
    # Convert JSON/CSV-like dictionaries into typed particles and tolerate common
    # alternate coordinate key names from upstream pickers.
    ...
    for item in source:
        ...
        if "x" not in item or "y" not in item:
            ...
            # continue with normalized fields below
        if "x" not in item or "y" not in item:
            raise ValueError("particle item missing required x/y fields")
...
def build_particle_records(
    ...
    """
    Build particle rows from a micrograph and coordinate list.

    Returns rows with extracted, edge-normalized particle data in each row.
    """
...
def create_particle_stack(particles: Sequence[ParticleStackRow], output_path: str) -> None:
    """
    Write normalized particle stack to MRCs file.

    If mrcfile is unavailable, raises a runtime error.
    """
    if len(particles) == 0:
    ...
    stack = np.stack([p.extracted_box for p in particles if p.extracted_box is not None], axis=0).astype(np.float32)
    if stack.shape[0] == 0:
...
def write_relion_star(
    ...
    """
    Write RELION-compatible metadata.
    """
    output = Path(output_path)
    output.parent.mkdir(parents=True, exist_ok=True)

    # RELION requires a consistent column header set across all rows.
    ...
    if micrograph_pixel_size_angstrom is not None:
        columns.append("_rlnPixelSize #15")
...
def write_json_output(rows: Sequence[ParticleStackRow], output_path: str) -> None:
    """Write JSON companion with normalized particle rows."""
    output = Path(output_path)
    output.parent.mkdir(parents=True, exist_ok=True)
...
def load_particles_from_csv(path: str) -> List[Dict[str, Any]]:
    ...
    if not rows:
        return []
    # normalize possible header style differences
    return rows


def normalize_micrograph_path(micrograph_path: str) -> str:
    return str(Path(micrograph_path).name)
...
def build_and_write(
    ...
    """
    End-to-end API entrypoint: build rows and write outputs.
    """
    coords = _as_particle_coords(particles)
    if not coords:
        rows: List[ParticleStackRow] = []
        write_relion_star(rows, star_output, stack_path=stack_output, micrograph_pixel_size_angstrom=config.micrograph_pixel_size_angstrom)
...
def _format_image_token(stack_path: Optional[str], index: int) -> str:
    if stack_path is None:
        return f"{index}"
    return f"{Path(stack_path).name}@{index:06d}"
