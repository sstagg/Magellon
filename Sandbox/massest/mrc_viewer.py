...
class ImageGridApp:
    def __init__(self, root, images_data):
        ...
        self.data_min = self.images_data[0].min()
        self.data_max = self.images_data[0].max()
        ...
        #self.items_per_page is subject to change if it goes out of bounds, this one won't change in that situation
    ...
    def debounce(self, func, delay):
        """Debounce function to limit how often 'update_grid' is called."""
    ...
    def update_info_grid(self, frame):
        """Update the info box with page and slice numbers"""
    ...
    def create_controls(self, frame):
        """Create input box, buttons, and scale controls.(bottom of main window)"""
        ...
        # Control window button (Right-aligned)
    ...
    def update_items_per_page(self, event):
        """changes how many images are being shown per page, 
        when changed it goes back to image #1"""
    ...
    def update_grid(self):
        """Update the grid layout. Called very frequently"""
        ...
        #logic for when ipp(items per page) is greater than the number of images
        #i.e. Setting ipp to 100 but there are only 25 images in the stack
        ...
        #loop through the images currently being displayed
        for i in range(self.first_slice_index,min(self.first_slice_index + self.items_per_page, len(images_data))):
            
            #Normalizes the data, and applies current brightness and contrast modifiers
            img = 128 + self.contrast / 50 * (((self.images_data[i] - self.data_min) / 
                                               (self.data_max - self.data_min) * (self.brightness * 5.1)).astype(np.float32) - 128)
            ...
            # Add the metadata overlay at the center-bottom
    ...
    def add_number_overlay(self, parent, number, position=(0.05, 0.05), bg="black", fg="white", pos = "nw"):
        """Overlay the slice's number in the stack"""
...
def parseArgs():
    ...
    Usage: python mrc_viewer.py --stackpath [filepath to mrc] --metadata [path to metadata file], [path to metadata file]..."""
