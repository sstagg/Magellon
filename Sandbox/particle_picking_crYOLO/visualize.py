#!/usr/bin/env python
"""
Quick visualizer: overlays crYOLO picks on the source micrograph.

crYOLO is robust on raw micrographs at high mag (the network includes its own
filtering pass), so unlike the topaz visualizer this defaults to the raw view.
A denoised-view option remains available if you want it.

Usage:
    python visualize.py <path_to_mrc_file> [--picks picks.json] [--output overlay.png]
                        [--top N] [--radius R] [--view raw|denoise]
"""
...
def _find_topaz():
    """Optional: borrow `topaz denoise` for the denoised-view fallback."""
...
def denoise_for_view(mrc_path, cache_path):
    ...
    if topaz_exe is None:
        raise RuntimeError("topaz CLI not found; install topaz-em or use --view raw.")
...
def main():
    parser = argparse.ArgumentParser(description="Overlay crYOLO picks on a micrograph")
    ...
    parser.add_argument('--radius', type=int, default=None,
                        help="Override box radius in pixels (useful when STAR lacks width)")
    parser.add_argument('--view', choices=['raw', 'denoise'], default='raw',
                        help="Background image (default: raw)")
    ...
    for p in picks:
        x, y = p['center']
        r = args.radius if args.radius is not None else p.get('radius', 0) or 50
