...
#
# Modified (contributed to) by Keenan Hom
...
class tkteach:
	...
	def initializeDatabase(self):
		...
		self.db = sq.connect(self.storage_path)
	...
	def initializeDatasets(self):
		...
		img_path = os.path.join(self.dir, 'images/')
		self.dataSetsListDir = [os.path.join(img_path, dataset) for dataset in os.listdir(img_path)]

		self.dataSetsListStr = [os.path.basename(d) for d in self.dataSetsListDir] #[x[2:] for x in self.dataSetsListDir]
...
if __name__ == "__main__":
	...
	if args.categories:
		assert os.path.isfile(args.categories), f'ERROR: {args.categories} is not a valid file'
	# If using default categories
	else:
