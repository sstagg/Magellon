"""Lazy-constructed step-event publisher for the MotionCor plugin.

Mirror of ``magellon_ctf_plugin/service/step_events.py``. Same env
toggles (``MAGELLON_STEP_EVENTS_ENABLED`` / ``MAGELLON_STEP_EVENTS_RMQ``)
so the operator gates both plugins from one place. ``PLUGIN_NAME`` /
        ...
        try:
            ...
            if _rmq_enabled():
                try:
                    ...
                    logger.info("MotionCor step-event publisher: RMQ mirror enabled")
                except Exception:
                    logger.exception("MotionCor step-event publisher: RMQ init failed — NATS only")
                    _rmq = None
            ...
            logger.info("MotionCor step-event publisher ready (transports=%d)", len(transports))
            return _publisher
        except Exception:
            logger.exception("MotionCor step-event publisher init failed — disabling for this run")
            _nats = None
            _rmq = None
            _publisher = None
