...
from plugin.ptolemy.PoissonMixture import PoissonMixture
...
class UNet_Segmenter:
    def __init__(self, channels, layers, model_path, threshold=0.0001, normalize=True, cuda=False, dim_mult_of=512):
        # `channels` / `layers` are kept for signature compatibility with
        # upstream ptolemy; they are encoded into the exported ONNX graph
        # so nothing needs to re-instantiate a BasicUNet here. `model_path`
        # must now point at an .onnx file.
        self.model = Wrapper(model_path, cuda=cuda)
