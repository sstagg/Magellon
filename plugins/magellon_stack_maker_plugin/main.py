"""Stack-maker plugin entry — slim FastAPI host for the broker runner.

Forked from FFT plugin. Production task flow:
RMQ → PluginBrokerRunner → StackMakerPlugin.execute → result queue.
HTTP surface: /health + Prometheus /metrics.
"""
...
from plugin import StackMakerPlugin, build_extraction_result


_plugin = StackMakerPlugin()
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        _runner = PluginBrokerRunner(
            ...
            result_factory=build_extraction_result,
            contract=PARTICLE_EXTRACTION_CATEGORY,
        )
        threading.Thread(
            target=_runner.start_blocking, name="stack-maker-broker-runner", daemon=True
        ).start()
    except Exception:
