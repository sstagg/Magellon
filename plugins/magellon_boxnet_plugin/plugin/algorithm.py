"""
BoxNet picker compute primitives.

`BoxnetPT` + `flexible_forward` are the verbatim onnx2torch graph trace
from eval_micrograph/boxnet_utils.py; numerically-validated against the
upstream by Sandbox/particle_picking_boxnet/parity_check.py. Do not
hand-edit the forward — a "readable" refactor swung particle_mean from
0.023 to 0.000 on great.png; keep the long generated form intact.

`pick_with_boxnet` is the picker entry point the plugin's compute layer
calls — downsample → BoxnetPT → particle channel → peak_local_max →
topaz/cryolo-shaped pick list.
"""
...
# Picker entry point
#
# BoxnetPT returns a (H, W, 3) softmax mask. Pickers need (x, y, score)
# centroids in original-image space. The two helpers below bridge that:
#
#   * ``_downsample`` block-mean downsamples by an integer factor —
#     matches Warp's training-time preprocessing (BoxNet was trained at
#     ~8 Å/pixel; a typical 1 Å/pixel micrograph needs ``scale=8``).
#   * ``pick_with_boxnet`` runs the full pipeline and emits a list of
#     pick dicts compatible with the topaz / crYOLO plugin output.
# ---------------------------------------------------------------------------


_DEFAULT_WEIGHTS_FILENAME = "boxnet.pt"


def default_weights_path() -> str:
    """Discover ``boxnet.pt`` from ``$BOXNET_WEIGHTS_DIR`` (Dockerfile
    sets this to ``/app/weights``) or fall back to a ``weights/``
    ...
    """
    candidates = []
    env_dir = os.environ.get("BOXNET_WEIGHTS_DIR")
    if env_dir:
        candidates.append(os.path.join(env_dir, _DEFAULT_WEIGHTS_FILENAME))
    candidates.append(
        os.path.join(
    ...
    raise FileNotFoundError(
        f"BoxNet weights not found. Tried: {candidates}. Drop boxnet.pt "
        f"into weights/ (or set BOXNET_WEIGHTS_DIR)."
    )
...
def _downsample(img: NDArray, scale: int) -> NDArray:
    """Block-mean downsample by an integer factor. ``scale=1`` is a no-op."""
    if scale == 1:
    ...
    return cropped.reshape(
        new_h // scale, scale, new_w // scale, scale
    ).mean(axis=(1, 3))
...
def pick_with_boxnet(
    ...
    *,
    ...
    Coordinates are in **original-image pixel space** (multiplied by
    ``scale`` on the way out). Each dict matches the topaz / crYOLO
    plugin shape::
