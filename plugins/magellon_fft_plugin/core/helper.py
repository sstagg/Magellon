...
from magellon_sdk.models import FftTaskData, TaskDto, TaskResultDto


def extract_task_data_from_object(task_object):
    return FftTaskData.model_validate(task_object.data)


def parse_json_for_ffttask(message_str):
    return FftTaskData.model_validate(TaskDto.model_validate_json(message_str).data)
