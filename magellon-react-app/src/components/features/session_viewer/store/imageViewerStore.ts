...
export type ViewMode = 'grid' | 'tree' | 'table';interface ImageViewerState {
    // Current selections
    currentSession: SessionDto | null;
    currentImage: ImageInfoDto | null;    // Navigation state - keeping your current ImageColumn structure
    imageColumns: ImageColumn[];    // Tab management
    activeTab: string;    // Atlas management
    isAtlasVisible: boolean;
    currentAtlas: AtlasImageDto | null;
    atlasImages: AtlasImageDto[] | null;    // Particle picking
    selectedParticlePicking: ParticlePickingDto | null;    // View modes
    viewMode: ViewMode;    // Image settings
    brightness: number;
    contrast: number;
    scale: number;    // Dialog states
    isParticlePickingDialogOpen: boolean;    // Actions
    ...
    contrast: 50,
    scale: 1,
    isParticlePickingDialogOpen: false,    // Actions
    setCurrentSession: (session) => set({
        currentSession: session,
        currentImage: null,
        imageColumns: [...initialImageColumns],
    }),    setCurrentImage: (image) => set({ currentImage: image }),    updateImageColumn: (columnIndex, columnData) => set((state) => ({
        ...
        if (columnIndex >= state.imageColumns.length - 1) return state;        const updatedImageColumns = state.imageColumns.map((column, index) => {
    ...
    }),    setAtlasImages: (atlasImages) => set({ atlasImages }),    setActiveTab: (tabId) => set({ activeTab: tabId }),    toggleAtlasVisibility: () => set((state) => ({
        isAtlasVisible: !state.isAtlasVisible
    })),    setCurrentAtlas: (atlas) => set({ currentAtlas: atlas }),    setSelectedParticlePicking: (pp) => set({ selectedParticlePicking: pp }),    updateParticlePicking: (updatedPP) => set({ selectedParticlePicking: updatedPP }),    setViewMode: (mode) => set({ viewMode: mode }),    setBrightness: (value) => set({ brightness: value }),    setContrast: (value) => set({ contrast: value }),    setScale: (value) => set({ scale: value }),    openParticlePickingDialog: () => set({ isParticlePickingDialogOpen: true }),    closeParticlePickingDialog: () => set({ isParticlePickingDialogOpen: false }),
}));