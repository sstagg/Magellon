...
- ``JobDto.create`` passing ``uuid=...`` to a field named ``id``.
...
# --- TaskResultDto: optional UUID fields + round-trip ---
...
def test_task_result_dto_round_trip_all_optional():
    ...
    r = TaskResultDto(code=200, message="ok")
...
def test_task_dto_job_id_defaults_per_instance():
    ...
    TaskDto() with a defaulted job_id shared the *same* UUID."""
    a = TaskDto(data={})
    b = TaskDto(data={})
...
def test_job_dto_create_sets_id_not_silently_drops_it():
    ...
    job = JobDto.create({"k": "v"}, CTF_TASK)
...
# --- CryoEmMotionCorTaskData.PixSize: False → None ---
...
def test_motioncor_pixsize_default_is_none():
    ...
    d = CryoEmMotionCorTaskData(inputFile="x.tif", Gain="g.mrc")
    assert d.PixSize is None


def test_motioncor_pixsize_accepts_float():
    d = CryoEmMotionCorTaskData(inputFile="x.tif", Gain="g.mrc", PixSize=0.705)
